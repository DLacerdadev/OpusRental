...
3. **Customer pays via card** — the public link `/pay/:token` (token built by `buildPublicPaymentUrl`) opens Stripe Elements without requiring a login. Use card `4242 4242 4242 4242`, any future expiry, any CVC and any ZIP for the test card.
4. **Webhook closes the invoice** — Stripe sends `payment_intent.succeeded` to `/api/stripe/webhook`. The handler validates amount + status, marks the invoice `paid`, **inserts a row in `invoice_payments`** (the dedicated ledger for customer payments), writes a `payment_validated` audit log, and dispatches an in-app notification ("Fatura paga …") to every manager/admin of the tenant via the WebSocket notification service. Failures during notification fan-out are swallowed so Stripe is never asked to retry an already-paid invoice.
5. **Verification** — the invoice flips to `paid` in `/financial`, a row appears in `invoice_payments` (queryable per invoice/tenant), the manager sees a real-time toast/notification, and `audit_logs` keeps both `payment_validated` (success) and `payment_rejected_amount`/`payment_rejected_status` (failure) entries for compliance review.

### Persistence model: two payment tables

- `payments` — share/investor distributions only. NOT NULL `shareId` + `userId` and a unique index on `(shareId, referenceMonth)`. Used by the monthly investor payout job. **Never** receives customer/invoice payments.
- `invoice_payments` — customer payments collected for invoices (Stripe today, PIX/manual reserved for future). Schema: `tenantId`, `invoiceId`, `amount`, `paidAt`, `method`, `stripePaymentIntentId` (UNIQUE — provides webhook idempotency: a re-delivered Stripe event triggers a Postgres unique violation that the handler catches and swallows so notifications and audit entries never duplicate).

The two tables are intentionally separate so the share distribution model keeps its strong invariants (one payment per share per month) while the customer-payment ledger evolves independently.

### Security & integrity guards

- **Webhook signature**: `/api/stripe/webhook` verifies the Stripe signature against the original raw request bytes (captured by `express.json({ verify })` in `server/index.ts` and exposed as `req.rawBody`). The handler refuses the request when the raw buffer is missing — required because the parsed JSON body would change byte ordering and fail HMAC validation.
